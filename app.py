...
AMENITY_GROUPS = {
    "🏥 Healthcare":     {"color": [204, 0,   0,   240], "themes": ["moh_hospitals", "vaccination_polyclinics", "registered_pharmacy", "blood_bank"]},
    "🏛️ Community":      {"color": [204, 153, 0,   240], "themes": ["libraries", "communityclubs", "ssot_hawkercentres", "eldercare", "family", "sso"]},
    "🌳 Parks & Nature": {"color": [0,   153, 0,   240], "themes": ["nationalparks", "sportsg_sport_facilities", "nparks_parks"]},
    "🎭 Culture":        {"color": [102, 0,   204, 240], "themes": ["museum", "theatre", "historicsites", "monuments"]},
    "🏨 Hotels":         {"color": [255, 140, 0,   240], "themes": ["hotels", "tourism"]},
    "🚒 Emergency":      {"color": [255, 50,  50,  240], "themes": ["firestation", "aed_locations", "civildefencepublicshelters"]},
}
...
# Amenity group toggles — only show when amenities is on and location searched
amenity_group_toggles = {}
if show_amenities:
    if center_lat:
        st.sidebar.markdown("**Amenity Groups**")
        for group_name in AMENITY_GROUPS:
            amenity_group_toggles[group_name] = st.sidebar.checkbox(group_name, value=True, key=f"am_{group_name}")
    else:
...
if show_mp:
    if not center_lat:
        st.warning("⚠️ Search a location first to load Master Plan for that area.")
    else:
        ...
        if mp_geojson:
            ...
            for f in mp_geojson.get("features", []):
                ...
                try:
                    ...
                    continue
                mp_data.append({"coordinates": coords, "lu_desc": lu, "gpr": gpr, "color": color})
            layers.append(pdk.Layer(
                "PolygonLayer", data=mp_data,
                get_polygon="coordinates", get_fill_color="color",
                get_line_color=[150, 150, 150, 80], get_line_width=1,
...
if show_gls:
    ...
    if gls_geojson:
        ...
        layers.append(pdk.Layer(
            "PolygonLayer", data=gls_polygons,
            get_polygon="coordinates", get_fill_color=[255, 140, 0, 100],
            get_line_color=[255, 100, 0, 200], get_line_width=2,
            pickable=True, stroked=True, filled=True,
        ))
        layers.append(pdk.Layer(
            "ScatterplotLayer", data=gls_dots,
            get_position=["longitude", "latitude"],
            get_fill_color=[255, 80, 0, 220], get_radius=40, pickable=True,
        ))
...
if show_tx and len(filtered) > 0:
    if tx_view == "Heatmap":
        layers.append(pdk.Layer(
            ...
            get_position=["longitude", "latitude"],
            get_weight="psf", radiusPixels=40, opacity=0.8,
        ))
    else:
        ...
        layers.append(pdk.Layer(
            "ScatterplotLayer", data=tx_map,
            get_position=["longitude", "latitude"],
            get_fill_color="color", get_radius=50,
...
if show_amenities and center_lat and onemap_token:
    ...
    for group_name, group_config in AMENITY_GROUPS.items():
        ...
        for item in items:
            ...
            amenity_data.append(item)
    if amenity_data:
        layers.append(pdk.Layer(
            "ScatterplotLayer", data=amenity_data,
            get_position=["longitude", "latitude"],
            get_fill_color="color", get_radius=35,
            pickable=True, opacity=0.9,
        ))
        st.sidebar.markdown(f"**{len(amenity_data):,} amenities**")
    else:
        st.sidebar.info("No amenities found in radius.")
...
if center_lat:
    ...
    layers.append(pdk.Layer(
        ...
        get_line_width=3, stroked=True, filled=True,
    ))
    layers.append(pdk.Layer(
        "ScatterplotLayer",
        data=[{"lat": center_lat, "lon": center_lon}],
        get_position=["lon", "lat"],
        get_fill_color=[50, 100, 255, 255], get_radius=30,
    ))
...
if show_mp and center_lat:
    with st.expander("Master Plan Legend"):
        ...
        for i, (lu, color) in enumerate(LAND_USE_COLORS.items()):
            ...
            cols[i % 4].markdown(
                f"<span style='background:{hex_color};padding:2px 8px;border-radius:3px;font-size:11px;color:{'#000' if sum(color[:3]) > 400 else '#fff'}'>{lu}</span>",
...
if show_tx and len(filtered) > 0:
    ...
    with tab1:
        trend = (
            filtered.groupby("date")["psf"].median()
            .reset_index().rename(columns={"date": "Date", "psf": "Median PSF"})
        )
    ...
    with tab2:
        vol = (
            filtered.groupby("date").size()
            .reset_index(name="Transactions").rename(columns={"date": "Date"})
        )
    ...
    with tab3:
        ...
        hist_df = pd.DataFrame({"PSF": edges[:-1].astype(int), "Count": hist})
    ...
    with tab4:
        by_type = (
            filtered.groupby("property_type")["psf"].median()
...
st.markdown("---")
st.caption("Data: URA Data Service API · Master Plan 2025 · data.gov.sg · OneMap · Updated Tue & Fri")
